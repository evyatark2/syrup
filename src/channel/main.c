...
static struct OnPacketResult on_client_packet(struct Session *session, size_t size, uint8_t *packet)
{
    ...
    case 0x00CA: {
        ...
        READER_END();
        struct Drop drop;
        if (map_remove_drop(room_get_context(session_get_room(session)), client->character.id, oid, &drop)) {
            switch (drop.type) {
            case DROP_TYPE_MESO:
                client_gain_meso(client, drop.meso.amount, true, false);
            break;
